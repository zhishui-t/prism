...
export function CodeGraphPage() {
  ...
  function formatQueryResult(res: Record<string, unknown>): string {
    ...
    if (Array.isArray(nodes) && nodes.length > 0) {
      parts.push(`\n▸ ${t('graph.queryNodeCount', { n: nodes.length })}`)
      for (const n of nodes.slice(0, 20)) {
        const label = n.label ?? n.id ?? '?'
        const file = n.file ?? n.src ?? ''
        parts.push(`  • ${label}${file !== '' ? `  (${file})` : ''}`)
      }
      if (nodes.length > 20) parts.push(`  … ${t('graph.queryMoreNodes', { n: nodes.length - 20 })}`)
    }
    const edges = res.edges as Array<Record<string, unknown>> | undefined
    if (Array.isArray(edges) && edges.length > 0) {
      parts.push(`\n▸ ${t('graph.queryEdgeCount', { n: edges.length })}`)
    }
    if (parts.length === 0) {
      const json = JSON.stringify(res, null, 2)
      return json.length > 2000 ? `${json.slice(0, 2000)}…${t('graph.truncated')}` : json
    }
    return parts.join('\n')
  }
  ...
  return (
    <div className="graph-page">
      ...
      {hasGraph && (
        <div className="card graph-query-card">
          <h3>{t('graph.query')}</h3>
          <div className="row">
            <input
              className="grow"
      ...
      */}
      {hasGraph && (
        <div className="card graph-studio-card">
