...
| 2 | 设计 | 队长 | 串行 | 需求要点 + 侦察结论 | **四文档链**（2026-09-17 用户定稿，缺一不进审核）：①需求设计文档 requirements.md（需求分析：场景/目标/非目标/业务验收）②软件设计文档 software-design.md（架构/模块改动/接口契约/数据模型/算法/错误处理/平台点）③规格文档 spec.md（逐条 Given/When/Then 可测验收）④api.md（涉及接口时） | 四份齐、链路可追溯（需求→设计→规格逐条对应） | 缺项/断链 → 回需求 |
