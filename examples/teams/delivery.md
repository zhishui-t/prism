...
**4 · 快轨（2026-09-17 增补，效率约定 v2——用户再催效率）**：
- **UI 轮默认快轨**：纯界面轮不设独立设计审核（任务书即设计）；仅数据丢失/并发/安全/
  安装类全量项走完整关
- **检视修复合一**：reviewer 对 ≤5 行小修直接改（记录进报告），仅 BLOCKER/大改派修
- **测试瘦身**：黑盒每轮截图上限 8 张（只拍用户新界面）；白盒轻量轮免矩阵派发，
  阶梯即门禁（矩阵仅全量轮）
- **队长非阻塞**：代理后台跑，等待窗口写交付/DAG/下一棒
- 代价边界：快轨砍重复确认不砍安全网——数据类必走全量检视（v11 全量检视抓出 3 个
  预存数据丢失 bug 即其价值）
