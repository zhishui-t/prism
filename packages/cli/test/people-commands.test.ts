...
describe('CLI 参数解析与 ~ 展开（design-v3 §5 P14）', () => {
  it('parseArgv 接受 --harness-root/--from/--to/--source/--model/--thought-level', () => {
...
describe('prism role/team/skill（F10 端到端；临时目录）', () => {
  ...
  beforeEach(async () => {
    ...
    // 导入源夹具与装配目标分离：harnessRoot 只作 skill install 默认目标与默认目录推导基准
    ...
    cleanup.push(home, harnessRoot, srcDir)
    lines = []
    ctx = {
      ...defaultContext({
        stdout: (line) => lines.push(line),
  ...
  it('import → list → validate → render → install → skill install 全链路', async () => {
    // import（默认 from=<harnessRoot>/agents，to=<home>/roles；目录式落盘）
    ...
    lines = []
    expect(await runCommand(ctx, ['role', 'install', 'dev-1', '--harness-root', harnessRoot])).toBe(0)
    expect(lines.join('\n')).toContain('已直接住在')
    expect(existsSync(join(harnessRoot, 'agents', 'dev-1.md'))).toBe(false) // 不再复制装配
...
`,
    ...
    lines = []
    expect(await runCommand(ctx, ['team', 'install', 'mini', '--harness-root', harnessRoot])).toBe(0)
    expect(lines.join('\n')).toContain('无需装配复制')
    expect(lines.join('\n')).toContain('已在受管位置')
...
`,
    ...
    lines = []
    expect(await runCommand(ctx, ['team', 'install', 'duo', '--harness-root', harnessRoot])).toBe(0)
    const output = lines.join('\n')
    // 新语义：成员已直接住在 roles_dir，无装配复制；researcher（非成员）与本命令完全无关
    expect(output).toContain('无需装配复制')
    expect(output).not.toContain('researcher')
    expect(existsSync(join(harnessRoot, 'agents'))).toBe(false) // 不产生任何角色复制
    expect(existsSync(join(home, 'roles', 'dev-1', 'AGENTS.md'))).toBe(true)
...
`,
    ...
    lines = []
    expect(await runCommand(ctx, ['team', 'install', 'ghost', '--harness-root', harnessRoot])).toBe(1)
    const output = lines.join('\n')
    expect(output).toContain('成员角色不在 roles_dir')
    expect(output).toContain('ghost-role')
    // 未产生任何写入（zcode 目录不被触碰）
    expect(existsSync(join(harnessRoot, 'agents'))).toBe(false)
  })
