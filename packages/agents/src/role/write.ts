...
import { renderMarkdownFile, splitFrontmatter, stripAllMarkerTails, type FrontmatterData } from '../frontmatter.js'
...
export interface RolePatch {
  ...
  /** 整段替换正文（其尾部 marker 会被剥掉——产物 marker 由末尾统一重挂一枚）。 */
  body?: string
}
 ...
 *
 * **marker 累积修复（v16 B-4 / R-4）**：两个正文入口（`splitFrontmatter` 的 body、
 * `patch.body`）**各自**就地 `stripAllMarkerTails()` 剥净尾部——overlay 注入因此永远发生在
 * 无 marker 正文上，末尾再 append 一枚 fresh marker，产出恒为「幂等单 marker」。修前旧 marker
 * 会被新注入的小节埋进中段（尾部正则够不着），替换**非末位**小节时每轮净 +1。
 * `hadMarker` 仍取 `raw.includes`（快照形态恒无 marker，改成「剥出过」会让 UI PATCH 掉 marker）。
 */
export function patchRoleRaw(raw: string, patch: RolePatch): string {
  ...
  // 入口①：splitFrontmatter 的正文就地剥尾 marker（可能有多枚旧累积）——正文必须无 marker
  // 再进 replaceSection，否则旧 marker 被新小节埋进中段，此后尾部正则再也够不着。
  let nextBody = stripAllMarkerTails(body).replace(/^\n+/, '')
  ...
  // 入口②：patch.body 同样就地剥尾 marker——UI 快照通常不带，但客户端若回传带 marker 的正文
  // （或带旧累积），不能让它把 marker 带进中段。
  if (patch.body !== undefined) nextBody = stripAllMarkerTails(patch.body)
