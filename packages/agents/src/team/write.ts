 ...
 *
 * **marker 累积修复（v16 B-4 / R-4）**：与 `role/write.ts` 同 helper 同口径——正文入口就地
 * `stripAllMarkerTails()` 剥净尾部，末尾重挂一枚 fresh marker。
 */
export function patchTeamRaw(raw: string, patch: TeamPatch): { markdown: string; issues: ValidationIssue[] } {
  ...
  // 入口：正文就地剥尾 marker（同 role/write.ts，见那里注释）。
  // 另存一份「只剥前导空行」的形态给 `bodyLineOffset` 当第三参：它靠 `body` 与该参的**长度差**
  // 反推正文的行号偏移（前导换行数 = 被剥掉的行数）。把尾部 marker 的长度算进去会让行号整体
  // 错位、表格区 splice 落到别的行上——而 marker 恒在文件最尾，剥不剥都不影响它之前任何一行。
  const bodyStripped = body.replace(/^\n+/, '')
  let nextBody = stripAllMarkerTails(bodyStripped)
  ...
  if (patch.members !== undefined) {
    ...
    if (patch.workflow !== undefined) {
      ...
      const parsed = parseWorkflowSection(nextBody, { baseLine: bodyLineOffset(raw, body, bodyStripped) })
      issues.push(
