...
describe('resolveDirs（prism.yaml 覆盖适配器默认；无配置 → 默认回落）', () => {
  it('无配置（null）→ 适配器默认：zcodeDir 推导 agents/teams/skills（teams 不在 agents 内），source=default', () => {
    const dirs = resolveDirs(null, { zcodeDir: 'K:/tmp/zcode' })
    expect(dirs).toEqual({
    ...
    // 缺省 zcodeDir → ZCode 适配器默认根（仅路径推导，不触碰磁盘）
    expect(resolveDirs().zcodeDir).toBe(DEFAULT_ZCODE_DIR)
    // B6：默认链（未显式指定 zcodeDir）→ 全部需要写守卫
    expect(resolveDirs(null).guard).toEqual({ roles: true, teams: true, skills: true })
  })

  it('显式 zcodeDir → guard 全放行（B6：用户显式指定可直接写）', () => {
    expect(resolveDirs(null, { zcodeDir: 'K:/tmp/zcode', zcodeDirExplicit: true }).guard).toEqual({
      roles: false,
      teams: false,
      skills: false,
    })
  ...
  it('配置覆盖生效（临时 roles_dir 等）；~ 与相对路径正确解释', () => {
    ...
    const relative = resolveDirs({ roles_dir: 'custom/agents' }, { zcodeDir: 'K:/tmp/zcode' })
  ...
  it('B6 guard 逐键判定：配置键放行；teams 缺省跟随已配置的 roles_dir 也放行', () => {
    const partial = resolveDirs({ roles_dir: '/tmp/custom-roles' }, { zcodeDir: 'K:/tmp/zcode' })
    ...
    const all = resolveDirs({ roles_dir: '/r', teams_dir: '/t', skills_dir: '/s' }, { zcodeDir: 'K:/tmp/zcode' })
  ...
  it('配置对象存在但无有效键 → 仍走默认值，source=config', () => {
    const dirs = resolveDirs({}, { zcodeDir: 'K:/tmp/zcode' })
...
describe('loadPrismConfig（<home>/prism.yaml 存在性）', () => {
  ...
  it('resolveDirsFromHome：端到端组合（临时 home 的配置覆盖 zcodeDir 默认）', () => {
    writeFileSync(
      join(home, 'prism.yaml'),
      ['roles_dir: /tmp/roles-x', 'teams_dir: /tmp/teams-x', 'skills_dir: /tmp/skills-x'].join('\n'),
    )
    const dirs = resolveDirsFromHome(home, { zcodeDir: 'K:/tmp/zcode' })
