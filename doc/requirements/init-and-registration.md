...
| **不覆盖** | 只增/改 `mcp.servers.prism`，其余键原样保留（`--force` 覆盖时亦然：宿主写的未知键不丢） |
| **冲突** | 已存在**且归一化后指向不同**才提示（键序、`node` vs `node.exe`、`\` vs `/`、缺省 `timeoutMs` vs `60000` 均归一，不误报）；`--force` 才覆盖 |
| **落点** | 正常接入走 `--yes` 写**默认宿主目录**；`--harness-root` 在 init 语境是**测试/CI 专用**（误指到项目目录后生成的 `cli/config.json` / `mcp.json` 可直接删除，宿主不读那里） |
