...
> **`team_id` 是团队的唯一标识（v12 F3）**：CLI/MCP/HTTP 与界面一律以它标识与寻址团队——
> 命令位置参数、路由 `:id`、文件名 `<team-id>.md` 用的都是它；`name` 只是人类可读的**元数据**，
> **界面展示面零展示**（列表/详情/工作流页头均显示 id；表单仍保留 name 输入，仅为该字段提供值）。
> 二者在 frontmatter 中的**读写契约不变**（保存往返不丢 name，见 §4.5）。

> **`extends` 是「合并视图」——GET 的字段可能混源（M-6）**：`extends` 让子团队继承父团队
> （父级作基底、子级**显式声明**的字段覆盖；数组字段子级声明即整体替换、不做隐式拼接；链深 ≤ 8、
> 成环报 `team_extends_cycle`；见 `packages/agents/src/team/extends.ts`）。故 `GET /api/teams/:id`
> 返回的是**合并结果**，字段**可能来自父级**；而 `workflow_raw` 恒为**被编辑文件本体**。两者不一致时
> 的后果（尤其 workflow）与「保存前以 `workflow_raw` 为准」的口径见 §4.5。
...
**已知限制（永久接受）：`if_match` 有毫秒级 TOCTOU 窗（M-9）**：写侧 `statSync`（取 mtime）→
`readFileSync` → `writeFile` 三步非原子（`agents/src/team/write.ts:154/165/166`）；读侧 `readTeamDetail`
更是**先 `readFile` 后 `statSync`**（`server/src/roles/team-read.ts:58/82`，危险方向：可产出「旧 raw + 新
mtime」的响应，使下一次 `if_match` 放行一次陈旧写）。**接受理由**：窗口是**毫秒级**、命中还须恰好卡在两步
之间，而 `if_match` 要防的「宿主手改」是**秒级的人手动作**；它本身定位为**乐观并发提示而非锁**，单用户本机
场景下攻击面≈0。**reopened 条件**：Prism 出现多进程/多用户共写同一团队目录，或 `if_match` 被赋予安全语义。
（注：团队文件写**不经** `SingleWriterQueue`——该队列只覆盖 core DB / 技能分类等面；故此处缓解来自窗口小与
单用户，不是串行化。）
...
| 4 | 沉淀规则是否写团队定义 | ✅ 是 |
| 5 | 团队定义是否可继承 | ✅ 支持 `extends`（合并视图：父作基底、子显式覆盖；链深 ≤8、成环报 `team_extends_cycle`——见 §2.1/§4.5 与 M-6 口径） |
